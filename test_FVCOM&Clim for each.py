...
for name in dict.keys(): #
    ...
    try:
        ...
        for i in FVCOM_csv.index:
            ...
            yrday=dtime.strftime('%j')
            Clim=getclim(lat1=lati,lon1=loni,yrday=yrday,var='Bottom_Temperature/BT_')
            ...
            for key in dict[name]['FVCOM_T']:
                if key == FVCOM_csv['time'][i]:
                    ...
                    dict[name]['Clim_T'][key]=float(Clim_list[i])
...
def getclim(lat1,lon1,yrday,var='Bottom_Temperature/BT_'):
#def getclim(lat1=43.,lon1=-73.0,yrday=dt.now().strftime('%j'),var='Bottom_Temperature/BT_'):
  ...
  inputdir='/home/jmanning/Mingchao/programe/clim/' # hardcoded climatology directory name "/home/pi/clim/"
